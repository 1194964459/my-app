...
函数式组件相比普通函数的区别：
函数式组件：驼峰命名 首字母大写，主要是为了渲染UI，返回 JSX/null，可使用Hooks
    输入为props，若无需参数，props 为空对象。
    返回值是React 元素（JSX）、null（不渲染任何内容）或 false（等价于 null），不能返回普通值（如数字、字符串，会报错）。

普通函数：驼峰命名 首字母小写，不返回UI，不能使用 Hooks
