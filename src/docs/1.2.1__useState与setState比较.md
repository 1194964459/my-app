...
相同点：
* 不可直接修改状态
* 都是异步更新，所以不能再更新后立即获得最新值；（若想获得更新后的值，setState需要第2个参数传函数；useState的状态值需要在useEffect中获取）
* 依赖旧值才能计算新值（useState、setState的参数必须是个函数）

不同点：   
* 是否合并状态？
  useState的更新函数不会自动合并状态，后设置的会覆盖之前的  导致之前的状态丢失；
  setState会合并“顶层属性”

* 状态值不变时：
  useState仅仅只触发一次渲染 后面值相同的话 不会再触发了；
  setState会重复触发，
