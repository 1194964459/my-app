...
Hooks管理的状态，是 React 维护的 “组件级持久化数据”，不会因函数组件的重新渲染而丢失。
React 为每个函数组件分配了一个独立的 Fiber 节点，Hooks 状态就存储在这个 Fiber 节点中，而非组件函数的局部变量（局部变量会随函数执行结束销毁）。
...
### 4. useReducer - 复杂状态管理
适合多子状态联动、多条件判断的场景。如：登录前、中、成功、失败、退出
